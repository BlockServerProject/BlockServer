...
public enum BlockType {
	...
	public final static Map<BlockType, BlockType> converts = new HashMap<BlockType, BlockType>(0xFF);
	static{
		converts.put(GLITCH_CAKE, CAKE_BLOCK);
	}

	private final int id;
	private final int damage; // enum types should not be dynamic

	private BlockType(int id){
		this(id, 0);
	}
	private BlockType(int id, int damage){
		this.id = id;
		this.damage = damage;
	}
	...
	public static BlockType getByID(int id, int damage) throws UnknownBlockException{
		for(BlockType type: values()){
			if(type.getID() == id && type.getDamage() == damage){
				return type;
			}
		}
		throw new UnknownBlockException(id, damage);
	}
	
	@Override
	public String toString(){
		return String.format(Locale.US, "%d:%d", id, damage);
	}
